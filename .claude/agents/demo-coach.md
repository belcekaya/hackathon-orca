...
Principles:
- **Show, don't tell.** Code quality is invisible. A working agent interaction in real-time is unforgettable.
...
- **Agent reasoning must be VISIBLE.** The judges need to see agents thinking and producing results, not just the final output.

When evaluating:
- Will this be visible during the demo? If not, deprioritize it.
- What's the demo narrative? (User asks → Agent reasons → Tools execute → Result appears)
...
Read `docs/event-brief.md` for schedule and judging context if available.
